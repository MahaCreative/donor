...
export default function EventDonor({ event}) {
    ...
    console.log(event);
    const {open: addModalButton, close:addModalClose, modal:addModalTrigger} = UseModal()
    const {open: editModalButton, close:editModalClose, modal:editModalTrigger} = UseModal()
    const {open: deleteModalButton, close:deleteModalClose, modal:deleteModalTrigger} = UseModal()
    const [dataModel, setDataModel] = useState([])
    const editModal = (data) => { editModalButton(), setDataModel(data)}
    const deleteModal = (data) => { deleteModalButton() , setDataModel(data)}
    const deleteHandler = () => {
    ...
    const reload = useCallback(
        debounce((query) => {
            Inertia.get(route('admin-event-donor'),
                query,
                {
                    preserveState:true
            },150)
    ...
    return (
        <div className='px-3 py-3 min-h-screen'>
            ...
            <Modal
                ...
                >
                <p className='text-white'>Apakah Anda Yakin ingin mengahpus data?</p>
                                                    ...
                                                    <Table.Td>
                                                        <Table.Dropdown>
                                                            <Menu>
                                                                ...
                                                                <Table.DropdownButton onClick={() => deleteModal(item)}>
                                                                    Delete
                                                                </Table.DropdownButton>
                                                            </Menu>
                                                        </Table.Dropdown>
